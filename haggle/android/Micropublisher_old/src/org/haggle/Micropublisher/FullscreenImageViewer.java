...
public class FullscreenImageViewer extends Activity implements ViewSwitcher.ViewFactory {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
        	Log.d(Micropublisher.LOG_TAG, "FullscreenImageViewer: Setting extras");
        	mPicturePaths = extras.getStringArray(PhotoView.KEY_PICTURE_PATHS);
        	Log.d(Micropublisher.LOG_TAG, "FullscreenImageViewer: Setting pos");
        	int pos = extras.getInt(PhotoView.KEY_PICTURE_POS);

        	Log.d(Micropublisher.LOG_TAG, "FullscreenImageViewer: Pos is " + pos);
        	...
        	Log.d(Micropublisher.LOG_TAG, "FullscreenImageViewer: width " + bmp.getWidth() + " height=" + bmp.getHeight());
	...
	protected void onStop() {
		...
		super.onStop();
		Log.d(Micropublisher.LOG_TAG, "onStop() in FullscreenImageViewer");
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (event.getX() - origTouchX > 0) {
				// Moved right
				Log.d(Micropublisher.LOG_TAG, "Swiped right");
				mSwitcher.showNext();
			} else {
				Log.d(Micropublisher.LOG_TAG, "Swiped left");
	...
	public boolean onTrackballEvent(MotionEvent event) {
		Log.d(Micropublisher.LOG_TAG, "MotionEvent");
