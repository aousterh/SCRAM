...
public class PhotoView extends Activity implements OnClickListener {
	...
	public static final String KEY_MICROPUBLISHER = "KeyMicropublisher";
	
	private ImageAdapter imgAdpt = null;
	private NodeAdapter nodeAdpt = null;
	private Micropublisher ps = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onCreate()");	
        ...
        ps = (Micropublisher)getApplication();
        ...
        try {
        	...
        	Log.d(Micropublisher.LOG_TAG, "set color failed: " + e);	
        }
        ...
        gallery.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int pos, long id) {
            	Log.d(Micropublisher.LOG_TAG, "Clicked image at pos " + pos);
    	...
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onCreate() done");	
    }
    
	@Override
    public void onRestart() {
    	super.onRestart();
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onRestart()");
    }
    ...
    public void onStart() {
    	...
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onStart() freemem=" +  Runtime.getRuntime().freeMemory());	
    	...
    	if (shouldRegisterWithHaggle) {
    		...
    		if (ret != Micropublisher.STATUS_OK) {
    			...
    			if (ret == Micropublisher.STATUS_SPAWN_DAEMON_FAILED) {
    				errorMsg = "Micropublisher could not start Haggle daemon.";
    			} else if (ret == Micropublisher.STATUS_REGISTRATION_FAILED) {
    				errorMsg = "Micropublisher could not connect to Haggle.";
    			}
    			Log.d(Micropublisher.LOG_TAG, "Registration failed, showing alert dialog");
    ...
    protected void onResume() {
    	super.onResume();
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onResume()");
	}
	@Override
	protected void onPause() {
    	super.onPause();
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onPause()");

 	}
    @Override
    protected void onStop() {
    	super.onStop();
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onStop()");
    }
    @Override
    protected void onDestroy() {
    	super.onDestroy();
    	Log.d(Micropublisher.LOG_TAG, "PhotoView:onDestroy()");
    }
    ...
    public void onClick(DialogInterface dialog, int which) {
    	Log.d(Micropublisher.LOG_TAG,"onClick: call finish()");
        finish();
    }
    ...
    protected Dialog onCreateDialog(int id) {
    	switch (id) {
    	case Micropublisher.STATUS_REGISTRATION_FAILED:
    		...
    		.setCancelable(false)
    		.create();
    	case Micropublisher.STATUS_SPAWN_DAEMON_FAILED:
    		return new AlertDialog.Builder(this)
    		.setTitle(R.string.haggle_dialog_title)
    		.setIcon(android.R.drawable.ic_dialog_alert)
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(0, MENU_INTERESTS, 0, R.string.menu_messages).setIcon(android.R.drawable.ic_menu_search);
...
//    	super.onOptionsItemSelected(item);877/((
    	...
    	case MENU_TAKE_PICTURE:
			...
			Log.d(Micropublisher.LOG_TAG, "filepath is " + filepath);
    	...
    	case MENU_INTERESTS:
        	...
        	this.startActivityForResult(i, Micropublisher.ADD_INTEREST_REQUEST);
        	
        	// THIS IS VERY HACKY
        	try {
    			// Try to send a primitive message
    			DataObject dObj = new DataObject();
    			dObj.addAttribute("Message", "Hi!", 1);
    			ps.getHaggleHandle().publishDataObject(dObj);
    			dObj.dispose();
    			Log.d(Micropublisher.LOG_TAG, "sent message!");
        	} catch (DataObjectException e) {
    			// TODO Auto-generated catch block
    			Log.d(Micropublisher.LOG_TAG, "Uh oh!!");
    		}
	...
	private void onPictureAttributesResult(int resultCode, Intent data) {
		...
		try {
			...
			for (int i = 0; i < attrs.length; i++) {
				Log.d(Micropublisher.LOG_TAG, "Picture has attribute " + attrs[i]);
	...
	/*		ps.getHaggleHandle().registerInterests(
			...
			Log.d(Micropublisher.LOG_TAG, "Disposed data object and " + aa.size() + " attributes");
		} catch (DataObjectException e) {
			// TODO Auto-generated catch block
			Log.d(Micropublisher.LOG_TAG, "Could not create data object for "
	...
	private void onAddInterestResult(int resultCode, Intent data) {
		...
		if (addedInterests != null && addedInterests.length != 0) {
			...
			for (int i = 0; i < addedInterests.length; i++) {
				...
				Log.d(Micropublisher.LOG_TAG, "Added interest " + addedInterests[i]);
			}
		...
		if (deletedInterests != null && deletedInterests.length != 0) {
			...
			for (int i = 0; i < deletedInterests.length; i++) {
				...
				Log.d(Micropublisher.LOG_TAG, "Deleted interest " + deletedInterests[i]);
			}
	...
	private void onImageCaptureResult(int resultCode, Intent data) {
		if (resultCode == Activity.RESULT_CANCELED) {
			Log.d(Micropublisher.LOG_TAG, "Image capture was canceled!");
			takenPicture = null;
			return;
		}
		if (resultCode != RESULT_OK) {
			takenPicture = null;
			Log.d(Micropublisher.LOG_TAG, "Image capture result was not OK!");
			return;
		}

		if (takenPicture == null) {
			Log.d(Micropublisher.LOG_TAG, "Taken picture path is null");
			return;
		}
		
		Log.d(Micropublisher.LOG_TAG, "Taken picture: " + takenPicture.getAbsolutePath());
		...
		this.startActivityForResult(data,
				Micropublisher.ADD_PICTURE_ATTRIBUTES_REQUEST);		
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
		case Micropublisher.ADD_PICTURE_ATTRIBUTES_REQUEST:
			onPictureAttributesResult(resultCode, data);
			break;
		case Micropublisher.ADD_INTEREST_REQUEST:
			onAddInterestResult(resultCode, data);
			break;
		case Micropublisher.IMAGE_CAPTURE_REQUEST:
			onImageCaptureResult(resultCode, data);
			break;
		default:
			Log.d(Micropublisher.LOG_TAG, "Unknown activity result");
		}
    }
	@Override
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (gallery.getSelectedView() == info.targetView) {
            ...
            Log.d(Micropublisher.LOG_TAG, "Target view is gallery");
    ...
    public class DataUpdater implements Runnable {
        ...
        public void run() {
    		Log.d(Micropublisher.LOG_TAG, "Running data updater, thread id=" + Thread.currentThread().getId());
        	switch(type) {
        	case org.haggle.EventHandler.EVENT_NEIGHBOR_UPDATE:
        		Log.d(Micropublisher.LOG_TAG, "Event neighbor update");
    ...
    class ImageAdapter extends BaseAdapter {
        ...
        public ImageAdapter(Context c, int width) {
        	...
        	Log.d(Micropublisher.LOG_TAG, "ImageAdapter constructor");
        ...
        final class ImageProcessingThread implements Runnable  {
			...
			{
				Log.d(Micropublisher.LOG_TAG, "Adding image to processing queue");
			...
			public void run() {
				...
				Log.d(Micropublisher.LOG_TAG, "Running image processing thread");
				...
				while (!shouldExit) {
		        	...
		        	Log.d(Micropublisher.LOG_TAG, "Processing new image in processing thread");
		        	...
		        	Log.d(Micropublisher.LOG_TAG, "time=" + System.currentTimeMillis() + " bitmap height=" + bmp.getHeight() + " width=" + bmp.getWidth());
        ...
        public synchronized void updatePictures(DataObject dObj) {
        	...
        	Log.d(Micropublisher.LOG_TAG, "Updating gallery images memfree=" + Runtime.getRuntime().freeMemory());
